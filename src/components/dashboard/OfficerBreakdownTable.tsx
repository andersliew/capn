import type { OfficerBreakdownRow } from "@/lib/types/dashboard";

type Props = {
  rows: OfficerBreakdownRow[];
};

export function OfficerBreakdownTable({ rows }: Props) {
  return (
    <div className="overflow-hidden rounded-xl border border-white/[0.06] bg-[#141419] shadow-sm">
      <div className="border-b border-white/[0.06] px-4 py-3">
        <h3 className="text-sm font-semibold text-zinc-100">Officer category breakdown</h3>
        <p className="text-xs text-zinc-500">
          Who is submitting reports by category
        </p>
      </div>
        ...
        <table className="w-full min-w-[820px] text-left text-sm">
          <thead className="sticky top-0 bg-[#141419] text-[11px] font-medium uppercase tracking-wide text-zinc-500">
            <tr>
              ...
              <th className="px-3 py-2.5 text-right">Routine</th>
              ...
              <th className="px-3 py-2.5 text-right">On-Demand</th>
            </tr>
          </thead>
            ...
            {rows.length === 0 ? (
              <tr>
                <td colSpan={8} className="px-3 py-12 text-center text-zinc-500">
                  No officer rows match your filters
                </td>
              </tr>
            ) : (
              rows.map((r) => (
                <tr key={r.officer} className="hover:bg-white/[0.02]">
                  <td className="max-w-[220px] truncate px-3 py-2.5 text-zinc-300" title={r.officer}>
                    {r.officer}
                  </td>
                  ...
                  <td className="px-3 py-2.5 text-right tabular-nums text-zinc-400">{r.routineReports}</td>
                  ...
                  <td className="px-3 py-2.5 text-right tabular-nums text-zinc-400">{r.onDemand}</td>
                </tr>
              ))
            )}
          </tbody>
